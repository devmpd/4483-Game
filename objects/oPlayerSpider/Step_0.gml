...
if(!global.paused){
	switch(form){
		...
		case 4:{
			...
			jumpsp = -8;
			grv = 0.4;
			sprite_walking = sSpiderWalking;
			sprite_standing = sSpiderStanding;
			sprite_jumping = sSpiderStanding;
			break;
		}
		default: {
			grv = 0.5;
	...
	if(!slinging){
		...
		sprite_index = sprite_walking;
		//Horizontal collision
		if(place_meeting(x+hsp, y, oSpiderPoint)){
			while(!place_meeting(x+sign(hsp), y, oSpiderPoint)){
				x = x + sign(hsp);		
			}
			hsp = 0;
			slinging = false;
		...
		//Vertical collision
		if(place_meeting(x, y+vsp, oSpiderPoint)){
			while(!place_meeting(x, y+sign(vsp), oSpiderPoint)){
				y = y + sign(vsp);		
			}
			vsp = 0;
			slinging = false;
