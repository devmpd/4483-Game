...
if(!global.paused){
	...
	if(place_meeting(x, y+1, oWater) && jump && form == 6){
		vsp = jumpsp;	
	}
	if(place_meeting(x, y+1, oWater) && form == 6 && !jump){
		vsp = 2;
	}

	if(place_meeting(x, y+vsp, oBottom) && form != 6)
	{
		room_goto(roomWater);
	}
	...
	if(place_meeting(x, y+1, oFloorWater) && form == 6){
	...
	if(place_meeting(x, y+vsp, oFloorWater))
	{
		...
		vsp = 0;	
	} else if (place_meeting(x, y+vsp, oBottom) && form == 6){
		vsp = 0;
	}
	...
	if(!place_meeting(x, y+1, oFloorWater) && form != 1 && form != 6)
	{
